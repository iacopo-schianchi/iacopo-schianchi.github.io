...
export default function Hero() {
      ...
      let typingInterval = setInterval(() => {
        ...
        setLines((prevLines) => {
          return prevLines.map((prevLine, index) =>
            index === lineIndex ? prevLine.concat(newChar) : prevLine
          );
        });
        ...
        if (lineIndex === 0 && currentLine.slice(0, i) === 'Hi,') {
          ...
          setTimeout(() => {
            typingInterval = setInterval(() => {
              ...
              setLines((prevLines) => {
                return prevLines.map((prevLine, index) =>
                  index === lineIndex ? prevLine.concat(nextChar) : prevLine
                );
              });
  ...
  return (
    <div className="hero" id="hero">
      ...
      <br />
      <h1 className={`title ${finished[1] ? 'finished' : ''}`} id="typing-text">
        {lines[1]}
      </h1>
    </div>
  );
